...
def create_zip():
    zip_filename = 'IBVAP_Full_Project.zip'
    # Only excluding dependencies that break on other PCs (.git, venv, node_modules, caches)
    # INCLUDED: yolov8n.pt, mediamtx, datasets, all code.
    exclude_dirs = {'.git', 'venv', 'node_modules', '__pycache__', '.pytest_cache', '.mypy_cache'}
    exclude_files = {zip_filename, 'IBVAP_Project.zip'}
